...
class WakeReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        object:AsyncTask<String, Unit, Boolean>() {
            override fun doInBackground(vararg p0: String): Boolean {
                ...
                val dests = getBroadcastAddresses()
            ...
            override fun onPostExecute(success: Boolean) {
                ...
                Toast.makeText(context, context.getString(if(success)R.string.packet_sent else R.string.packet_error, wp.mac), Toast.LENGTH_SHORT)
                    .show()
            }
    ...
    companion object{
        ...
        fun getBroadcastAddresses(): MutableList<InetAddress> {
            ...
            for(ni in NetworkInterface.getNetworkInterfaces()){
                for(addr in ni.interfaceAddresses){
                    addr.broadcast?:continue
                    out.add(addr.broadcast)
                }
            }
            return out
        }
        ...
        fun stringToMAC(str: String): ByteArray{
             val inpt = str.toUpperCase().replace('-', ':').split(':')

            val out = ByteArray(6)
            for(i in 0 until 6){
                out[i] = Integer.decode('#'+inpt[i]).toByte()
